...
public final class EconProgression implements Saveable {
    ...
    // === GEBAEUDE-CACHE (extracted to EconProgressionBuildingCache) ===
    final EconProgressionBuildingCache buildingCache = new EconProgressionBuildingCache();
    ...
    public void update(EconSnapshot snap) {
        ...
        // Milestones setzen
        buildingCache.pollBuildings();
        if (buildingCache.cachedStockpileCount > 0) msFirstStockpile = true;
        if (cumulativeExportValue > 0)              msFirstExport    = true;
        if (buildingCache.cachedTavernCount > 0)    msFirstTavern    = true;
        if (buildingCache.cachedMarketCount > 0)    msFirstMarket    = true;
        if (buildingCache.cachedTempleCount > 0)    msFirstTemple    = true;
        if (buildingCache.cachedLabCount > 0)       msFirstLaboratory = true;
        if (buildingCache.cachedMilitaryCount > 0)  msFirstMilitary  = true;
        if (buildingCache.cachedEmbassyCount > 0)   msFirstEmbassy   = true;
        ...
        switch (stage) {
            ...
            case INDUSTRIE:
                advance = snap.people >= 150
                    && msFirstLaboratory
                    && buildingCache.cachedLibraryCount > 0
                    && msFirstMilitary
                    && stageDays >= 30;
                break;
            case WOHLSTAND:
    ...
    /** Delegates to {@link EconProgressionBuildingCache#reliableStockpileCount()}. */
    public static int reliableStockpileCount() {
        return EconProgressionBuildingCache.reliableStockpileCount();
    }

    /** Reset all mutable state. Milestones (ms*) are preserved across resets. */
    public void clear() {
        stage = Stage.SUBSISTENZ;
        stageDays = 0;
        cumulativeWagesPaid = 0;
        cumulativeExportValue = 0;
        daysSinceInsolvency = 0;
        daysLowGini = 0;
        daysVeryLowGini = 0;
        daysStableTreasury = 0;
        lastTreasuryBalance = -1;
        statusLowInequality = false;
        advanceCooldownTicks = 0;
        buildingCache.clear();
    }
