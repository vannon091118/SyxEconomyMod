...
public final class ServiceMarket {
    ...
    public boolean admit(Humanoid humanoid, Wallets wallets, RoomServiceAccess service, Roster roster, Fiscal fiscal, FirmLedger ledger) {
        ...
        int urgency = service.need instanceof NEED_E ? Math.max(0, Math.min(4, hum.getEventNeedPriority(humanoid, service.need))) : (int)Math.ceil(4.0 * (1.0 - hum.getServiceFulfilment(humanoid, (StatService)service.stats())));
