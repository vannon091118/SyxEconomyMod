...
public final class EconConfig {
    ...
    // BA-02: Progressive wealth tax surcharge during extreme inequality.
    public static double giniWealthSurchargeThreshold = 0.80;
    public static double giniWealthCapMultiplier = 50.0;
    public static int giniWealthSurchargeRateBp = 500; // 5% of excess wealth

    // BA-02-Factors: Multi-factor surcharge modifiers.
    // Each factor scales the base surcharge multiplicatively.
    // Factor > 1.0 = higher surcharge, < 1.0 = lower surcharge.

    /** Property surcharge: citizens owning firm shares or homes pay
     *  this multiplier per property type above the threshold.
     *  0.0 = disabled (legacy behavior). */
    public static double giniPropertySurchargeFactor = 0.10;
    /** Minimum shares a citizen must own before the property surcharge kicks in.
     *  Prevents taxing citizens who own 1 share in 1 firm. */
    public static int giniPropertySurchargeMinShares = 3;
    /** Debt relief: citizens with debt > debtReliefThreshold × wealth
     *  get a surcharge reduction. 0.0 = disabled. */
    public static double giniDebtReliefFactor = 0.50;
    /** Scarcity reduction: when maxScarcity exceeds this threshold,
     *  surcharge is reduced by (1 - scarcityReduction × scarcity).
     *  Protects the rich from being taxed when the whole settlement is starving. */
    public static double giniScarcityReduction = 0.40;
    /** Firm profit surcharge: citizens owning shares in firms with
     *  total profit > firmProfitThreshold get this multiplier.
     *  0.0 = disabled. */
    public static double giniFirmProfitSurchargeFactor = 0.15;
    /** Total firm profit per day above which the firm-profit surcharge activates. */
    public static double giniFirmProfitThreshold = 100.0;
    ...
    // ─── Tick-Stagger-Intervalle (Performance-Optimierung) ───────────────
    // Steuern wie oft teure O(N)-Roster-Iterationen laufen.
    // At ~300 ticks/day: taxesStaggerTicks=2 → 150×/day, marketStaggerTicks=3 → 100×/day.
    // Höhere Werte = weniger CPU, aber langsamere Reaktion auf Markt-Änderungen.
    // Setze 1 für jedes-Tick-Verhalten (Legacy/Debug).

    /** Taxes + Gini-Surcharge: alle N Ticks (roster-Iteration). */
    public static int taxesStaggerTicks = 2;
    /** DebtBondage + Housing + Corvee + Maintenance + Labor + Oddjob: alle N Ticks. */
    public static int marketStaggerTicks = 3;
    /** Religion + Liturgy + HandoutRelief: alle N Ticks. */
    public static int religionStaggerTicks = 5;
    ...
    // —— L-03 WealthRest (reiche Bürger arbeiten Teilzeit) ————————————
    /** Aktiviert den WealthRest-Mechanismus: Bürger mit relativeWealth >
     *  wealthRestMedianMultiplier × Median bekommen einen niedrigeren
     *  Fatigue-Threshold → STAMINA-Malus greift früher → Engine vergibt
     *  weniger Arbeitspriorität → indirekte Teilzeit. */
    public static boolean wealthRestEnabled = true;
    /** Schwellenwert-Multiplikator über Median. Bürger mit relativeWealth
     *  > diesem Wert × Median bekommen den reduzierten Fatigue-Threshold. */
    public static double wealthRestMedianMultiplier = 3.0;
    /** Faktor um den der Fatigue-Threshold für reiche Bürger skaliert wird.
     *  < 1.0 = Threshold sinkt → STAMINA-Malus greift bei weniger Fatigue
     *  → reiche Bürger werden früher gedrosselt → arbeiten weniger. */
    public static double wealthRestThresholdFactor = 0.5;
