...
public final class WindowOverview extends EconWindowBase {
    ...
    private static final double CHAIN_IMPACT_THRESHOLD = 0.1;
    ...
    private static final class AdvisorTab implements TabContent {
        ...
        public void build(EconomySim sim, GuiSection content, int x, int y, int w, int h) {
            ...
            GText adviceText;
            if (advice.length() > 80) {
                adviceText = new GText(UI.FONT().S, FONTW_BODY);
            } else {
                adviceText = new GText(UI.FONT().M, FONTW_BODY);
            }
    ...
    private static String buildAdvice(EconomySim sim, WealthStats stats, EconIndicators ind, long treasury) {
        ...
        if (signal != null && sim.ioMatrix() != null && sim.ioMatrix().isValid()) {
            ...
            for (int i = 0; i < sim.ioMatrix().size() && i < init.resources.RESOURCES.ALL().size(); ++i) {
                if (sim.flowPrices().coverage(i) >= 0.5) continue;
                int affected = countChainAffected(sim, i);
                if (affected > worstChain) {
                    worstChain = affected;
                    init.resources.RESOURCE res = (init.resources.RESOURCE) init.resources.RESOURCES.ALL().get(i);
    ...
    /**
     * Zählt wie viele Ressourcen über die Leontief-Kette von Ressource {@code resourceIdx}
     * betroffen sind (Total-Koeffizient > {@link #CHAIN_IMPACT_THRESHOLD}).
     * Nutzt {@code computeTotalRequirements()} mit einem Einheits-Nachfrage-Schock.
     *
     * @return Anzahl der kettenbetroffenen Ressourcen (exkl. resourceIdx selbst)
     */
    private static int countChainAffected(EconomySim sim, int resourceIdx) {
