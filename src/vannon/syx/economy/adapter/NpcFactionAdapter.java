...
public final class NpcFactionAdapter implements ISyxNpc {
    ...
    private static final ClassResolver classResolverForNpc;
    static {
        ClassLoader cl = Humanoid.class.getClassLoader();
        GAME_CL = cl != null ? cl : ClassLoader.getSystemClassLoader();
        classResolverForNpc = new ClassResolver(GAME_CL);
    }
    ...
    public NpcFactionAdapter() {
        ...
        boolean ok = false;
        try {
            Class<?> frClass = classResolverForNpc.resolve(FACTION_RESOURCE_CLASS);
    ...
    /** Get FactionResource for a specific NPC faction (uses ClassResolver for version-safe access). */
    private Object getFactionResource(FactionNPC npc, String resourceKey) {
        try {
            ...
            // can't guarantee it. Use ClassResolver for version-safe access.
            Object resObj = npc.res();
            if (resObj == null) return null;
            // Use ClassResolver.invokeMethod which walks the hierarchy
            return classResolverForNpc.invokeMethod(resObj, "get", resource);
        } catch (Throwable t) {
